...
function chatbot() {
    return {
        ...
        basicMarkdown(text) {
            ...
            html = html.replace(/^##\s+(.+)$/gm, '<h4 class="chat-md-h">$1</h4>');
            // Numbered lists
            html = html.replace(/^\d+\.\s+(.+)$/gm, '<li class="chat-md-li">$1</li>');
            html = html.replace(/(<li[^>]*>.*?<\/li>\n?)+/g, function(m) {
                return '<ol class="chat-md-ol">' + m + '</ol>';
            });
            // Bullet lists
            html = html.replace(/^[-*]\s+(.+)$/gm, '<li class="chat-md-li">$1</li>');
            html = html.replace(/(<li[^>]*>.*?<\/li>\n?)+/g, function(m) {
                // Don't double-wrap if already inside <ol>
                if (m.includes('<ol') || m.includes('<ul')) return m;
                return '<ul class="chat-md-ul">' + m + '</ul>';
            });
            // Line breaks
            html = html.replace(/\n/g, '<br>');
            return html;
